...
define(['common','util','webaudio',"musicList"],function(c,u,w,list){
        ...
        LMusic.prototype ={
            config:{
                ...
                showrightmenu:false,
                gradientleft:'#52b228',
                gradientmiddle:'#55bd27',
                gradientright:'#52b228',
                gradienttop:'#f5f325',
            ...
            resetPlayer:function(idx){
                ...
                this.musicDom.cover.innerHTML = '<img  id="music-cover-img" class="tips" src="'+nowMusic.cover+'" data-direction="top" data-title="'+nowMusic.title + ' -- '+ nowMusic.singer + '">';
                //这里是新增功能能够根据页面音乐cover自动切换
                jQuery("#bg_musc_pause,.music-container-blur-bg").css('backgroundImage','url('+nowMusic.cover+')').data('title',nowMusic.title);
                //jQuery('.music-container-blur-bg').css('backgroundImage','url('+nowMusic.cover+')');
                var img = document.getElementById('music-cover-img');
                var colors = RGBaster.colors(img, {
                    paletteSize: 10,
                    success: function(colors){
                        img.style.borderColor = colors.secondary;
                        this.config.gradientleft = colors.palette[1];
                        this.config.gradientmiddle = colors.palette[2];
                        this.config.gradientright = colors.palette[3];
                        this.config.gradienttop = colors.dominant;
                        jQuery('.m-music-list-wrap,.m-music-lyric-wrap').css({backgroundColor:colors.dominant.replace(/rgb\((.*)\)/,'rgba($1,0.2)')});
                    }.bind(_this)
                });
                ...
                _callback.index = idx;
                var _this = this;
                typeof this.config.callback === 'function' && this.config.callback(_this);
            },
            /**
             * 控制音量播放
             * @AuthorHTL
            ...
            hideProgress:function(target){
                ...
                if(target.config.autoPlay){
                    target.play();
                    jQuery('#bg_musc_pause').toggleClass('rotate');
                }else{
                    target.pause();
                }
            ...
            action:function(){
                ...
                if(!this.isactionbind){
                    var enterFrame = function(){
                        ...
                        jQuery('.grid-music-container .u-process').data('title',currenttime);
                        //歌词进度
                            ...
                            for(;i<sizes;i++){
                                ...
                                if(lyl){
                                    ...
                                    if(curTime >= _time){
                                        ...
                                        _this.musicDom['lyricWrap'].style.marginTop = -top/20 +'rem';
                                        //移除之前的current
                        ...
                        timeid=requestAnimationFrame(enterFrame);
                    };
                    timeid=requestAnimationFrame(enterFrame);
                    //音量控制器 静音和回复播放
            ...
            renderLyric:function(lyric){
                ...
                if(lyric){
                    for(var k in lyric){
                        ...
                        tpl += '<li class="u-lyric f-toe tips" data-time="'+k+'" data-title="'+txt+'" data-direction="top">'+txt+'</li>';
                    }
             ...
             */
            tooglembtn:function (event){
                var _this = this;
                if(event){
                    event.preventDefault();
                    event.stopPropagation();
                }
                jQuery('#grid-music-container-3d').slideToggle();
                //var span = this.musicDom.m_music_btn.querySelector('span');
                //if(span.classList.contains('pause')){
                //    span.classList.remove('pause');
                //    span.classList.add('play');
                //}else{
                //    span.classList.remove('play');
                //    span.classList.add('pause');
                //
                //}
            },
            init:function(){
                ...
                //绑定移动端播放按钮
                this.musicDom.m_music_btn = document.querySelector('.myMusic');
                this.musicDom.m_music_btn.addEventListener('click',this.tooglembtn.bind(this));
